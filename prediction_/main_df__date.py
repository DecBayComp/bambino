...
def main(arg_str):
    ...
    arg_parser.add_argument('-b', '--hunch_weak',action='store', type=str)
    ...
    path_out = 'output/not_all_dates/new_other/'
    os.makedirs(path_out+'/'+Line ,exist_ok=True)

    tresh_hunch = 0.6
    static_tresh_one = 1 - 0.2
    static_tresh_two = 0.75

    Date_name = '01_12_thresh_'+str(round(1-static_tresh_one,2)).split('.')[-1]+'_'+str(static_tresh_two).split('.')[-1]+'_threshold_'+str(tresh_hunch)+'_norunHC'
    t_start = 0.0
    t_end = 120.0
    ...
    with open('CLF_BBH_19_07_2023.pkl', 'rb') as handle:
        CLF_BBH = pickle.load(handle)
    ...
    path_=var.path
    print(Line)
 #   if os.path.exists(path_out+Line+'/Ethogramme_'+ Line.split('/')[0]+'_'+ Line.split('/')[-1] + Date_name+'_'+str(id_number)+'.pdf'):
    ...
    if hunch_weak == True:
        ...
        Data_predic  = pd.concat([Data_hunch,Data_bend,back_crawl_large],axis=1)
    ...
    Data_predic = Data_predic.loc[Data_predic['Date'].isin(Date_)]
    #print(Date_, set(Data_predic['Date']))
    for date in Date_:
        print('date',date)
   ...
   # DF_ball.to_csv(path_out+'/'+Line+'/Ball_'+Line.replace('/','_').replace('@','_')+Date_name+'_'+str(id_number)+'.csv')
    ...
    for clf in CLF_BBH:
        ...
        DF_prediction['test'+str(str(CLF_BBH.index(clf)))]=pd.Series(list(y_pred),index=list(X_pred.index))
    ...
    # DF_before = df_behav_before.T

    # DF_before = pd.concat([DF_before.iloc[:,-3:],DF_before.iloc[:,:-3]],axis=1)
    # Col = ['Line','date','larva']+[i/10 for i in DF_before.columns[3:]]
    # DF_before.columns = Col
    # DF_before = DF_before.iloc[:,:1203]
    # #DF.to_csv(path_out + Line +'/Larva_behaviour_time_'+Line.split('/')[0]+'_'+ Line.split('/')[-1]+Date_name+str(id_number)+'.csv')

    # #print(DF)
    # DF_before_ = DF_before.iloc[:,4:]
    # df_proba_before = pd.DataFrame(np.zeros((len(labels_),len(DF_before_.columns))), index=labels_, columns=DF_before_.columns)

    # for i in DF_before_.columns[:]:
    #     df_proba_before.loc[:,i] = DF_before_.loc[:,i].value_counts()/DF_before_.loc[:,i].count()
    # df_proba_before.to_csv(path_out+Line+'/Proba_data_'+Line.split('/')[0]+'_'+ Line.split('/')[-1]+ Date_name+'_'+str(id_number)+'_OLD.csv')

    # fig = plt.figure(figsize=(6, 6))

    # ax = fig.add_subplot(111)
    # for labels__ in labels_:
    #     df = df_proba_before.T[labels__]
    #     df.plot(c=Color[int(labels_.index(labels__))], linewidth=2.5, label= labels__)
    # ax.set_ylim(0, 1)
    # ax.set_xlabel(r't')
    # ax.set_ylabel(r'p')
    # ax.set_xlim([0,120])
    ...
    # plt.title(str(Date_))
    # #ac+'/'+ Line +
    # fig.savefig(path_out+Line+'/Probability_'+Line.split('/')[0]+'_'+ Line.split('/')[-1] + Date_name+'_'+str(id_number)+'_OLD.pdf')



    #print(Prediction_after_correction['prediction'].value_counts())
        ...
        for larva in list(set(Features_by_date['Larva'])):
            ...
            for group in group_time:    
                ...
                try :
                    ...
                    print('error: no Angle') 
                    Features_mean.loc[idx, 'Angle max'] = 0                      
                ...
                # Liste_time_bend = Liste_time[idx_time]
                # Liste_angle = [np.arctan2(y_center[i]-y_head_after[i],x_center[i]-x_head_after[i])-np.arctan2(y_center[i]-y_head[i],x_center[i]-x_head[i]) for i in range(len(x_center)-1)]
                # Delta_time = [Liste_time_bend[1:][i]-Liste_time_bend[:-1][i] for i in range(len(Liste_time_bend)-1)]
                # Velocity_angle = [abs(Liste_angle[i])/Delta_time[i] for i in range(len(Delta_time))]            



                # Features_mean.loc[idx, 'Velocity mean'] = np.mean([abs(i) for i in Velocity_angle])
                # Features_mean.loc[idx, 'Velocity mean'] = np.mean([abs(i) for i in Velocity_angle])
                
                # Features_mean.loc[idx, 'number_time_step'] = len(list(Velocity_angle))
                # Features_mean.loc[idx,'Velocity_angle'] = str(list(Velocity_angle))

                try:
                    # print(f[trx['global_state_large_state'][0][nb_larva]][0][idx_time[-1]+1])
                    if  f[trx['global_state_large_state'][0][nb_larva]][0][idx_time[-1]+1] in [1.0,5.0]:
                        Features_mean.loc[idx, 'behav'] = 'Head-Cast'
                except:
                    print('error: no behavior after')
    ...
    for idx in Features_mean.index:
        ...
        #np.mean(Features_mean['velocity_before_head']))
    DF_prediction_bend = Features_mean.loc[:,['Larva', 'Line', 'Date','time_start', 'time_end','prediction','Angle max']]
    DF_prediction_bend.to_csv(path_out+Line+'/PREDICTION_Bends_'+Line.replace('/','_').replace('@','_')+Date_name+str(id_number)+'.csv')

    DF_prediction_bend = DF_prediction_bend.replace('Head-Cast slow','Head-Cast')
    ...
    #df_behav.T.to_csv(path_out + Line +'/Larva_behaviour_time_1_'+Line.split('/')[0]+'_'+ Line.split('/')[-1]+Date_name+str(id_number)+'.csv')
    ...
    #df_behav.T.to_csv(path_out + Line +'/Larva_behaviour_time_2_'+Line.split('/')[0]+'_'+ Line.split('/')[-1]+Date_name+str(id_number)+'.csv')
